...
function removeFirstOccurrences(str, value) {
  const valIdx = str.indexOf(value);
  if (valIdx === -1) return str;
  return str.slice(0, valIdx) + str.slice(valIdx + value.length);
}
/**
...
function removeLastOccurrences(str, value) {
  const valIdx = str.lastIndexOf(value);
  if (valIdx === -1) return str;
  return str.slice(0, valIdx) + str.slice(valIdx + value.length);
}
...
function invertCase(str) {
  ...
  return str
    .split(' ')
    .map((s) => [...s].map(invertCharacter).join(''))
    .join(' ');
}
...
function encodeToRot13(str) {
  ...
  let newStr = '';
  for (let i = 0; i < str.length; i += 1) {
    const strIdx = alphabet.indexOf(str[i]);
    if (strIdx === -1) {
      newStr += str[i];
    } else {
      newStr += rotated[strIdx];
    }
  }
  return newStr;
}
